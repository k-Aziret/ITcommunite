...
class PostDetailView(DetailView):
    template_name = "post_details.html"
    model = Post
    queryset = Post.objects.filter() # queryset - достает 
    context_object_name = "post"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["form"] = PostUpdateForm(instance=self.get_object()) #instance помогает получить описание, и изменить его (можно и без него)
...
class FollowingPostsView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        posts = Post.objects.filter(author__in=following, is_archive=False)
        return posts
@login_required
